        ...
        $.each(data, function (index, item) {
            ...
            // if (!item.status) {
                ...
                <div class="col-xl-1">
                    ...
                    title="${item.status ? "Đã có khách" : "Bàn trống"}"
                        type="button"
        ...
        $(".desk_option").prepend(
            `
                    <div class="row" style="margin-left: 10px">
                            <h2>${deskResp.name}</h2>
                            <button onclick="changerStatus(${deskResp.id})" style="width: 75px; text-align: center" title="${deskResp.status ? "Hủy bàn" : "Mở bàn"}"  class="btn btn-primary"><i class="fa fa-exchange-alt"></i></button>
...
//                         title="${item.status ? "Đã có khách" : "Bàn trống"}"
//                         style="border-radius: 35%; width: 100px; margin-left: 10px"
//                         class="btn btn-${item.status ? "danger" : "success"}"> ${item.name}</button></div>
//                             `
//             )
//
//             // if (!item.status) {
